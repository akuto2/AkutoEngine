	...
	protected boolean set(TileFillerEX fillerEX, ItemStack stack) {
		...
		while(fillerEX.cz >= fillerEX.sz && fillerEX.cz <= fillerEX.ez) {
			while(fillerEX.cx >= fillerEX.sx && fillerEX.cx <= fillerEX.ex) {
				if(isFilling(fillerEX.getWorldObj(), fillerEX.cx, fillerEX.cy, fillerEX.cz) && Math.abs(fillerEX.cx - fillerEX.mx) % AkutoEngine.intervalTorch == 0 && Math.abs(fillerEX.cz - fillerEX.mz) % AkutoEngine.intervalTorch == 0) {
