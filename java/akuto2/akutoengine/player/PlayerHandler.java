...
public class PlayerHandler {
	...
	public void onPlayerLogin(PlayerLoggedInEvent event) {
		if(AkutoEngine.update != null) {
			AkutoEngine.update.notifyUpdate(event.player, Side.CLIENT);
		}
	}
	...
	public void onCrafting(ItemCraftedEvent event) {
		...
		if(!event.player.worldObj.isRemote) {
			if((item == Item.getItemFromBlock(AkutoEngine.fillerEX))) {
				PlayerStats stats = PlayerStats.get(event.player);
				if(!stats.fillerManual) {
					stats.fillerManual = true;
					AbilityHelper.spawnItemAtPlayer(event.player, new ItemStack(AkutoEngine.manualFiller));
				}
			}
		}
	}
